export default async function handler(req, res) {
  ...
  try {
    const body = req.body;
    if (!body || !body.action) return res.status(400).json({ error: 'Action required' });
    ...
    if (body.action === 'add') {
      if (!body.service || !body.link || !body.quantity) return res.status(400).json({ error: 'Service, link, quantity required' });
      if (!body.link.startsWith('http')) return res.status(400).json({ error: 'Invalid link' });
      if (parseInt(body.quantity) < 100) return res.status(400).json({ error: 'Minimum 100' });
